...
const SWASTHA_CHAPTERS = [
    ...
    "Chapter 2 - Healthy Life Style - Dinacharya",
    ...
    "Chapter 11 - Janapadodhwamsa / Maraka Vyadhi",
    "Chapter 12 - Environmental Health",
    "Chapter 13 - Disaster Management",
    "Chapter 14 - Occupational Health",
    "Chapter 15 - School Health Services",
    "Chapter 16 - Disinfection",
    "Chapter 17 - Primary Health Care",
    "Chapter 18 - Mother and Child Health Care",
    "Chapter 19 - Family Welfare Programme",
    "Chapter 20 - Preventive Geriatrics",
    "Chapter 21 - WHO and International Health Agencies",
    "Chapter 22 - Vital Statistics",
    "Chapter 23 - Health Administration",
    "Chapter 24 - National Health Programmes",
        ...
        <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button
                        onClick={tab === "tests" ? openTestForm : openQuestionForm}
                        style={primaryBtn}
                    >
            ...
            {tab === "questions" && (
                <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
                    <select
                        ...
                        onChange={e => setQFilter(f => ({ ...f, subject: e.target.value }))}
                        style={smallSelect}
                    >
                    ...
                    <select
                        value={qFilter.term}
                        onChange={e => setQFilter(f => ({ ...f, term: e.target.value }))}
                        style={smallSelect}
                    >
                    ...
                    <select
                        value={qFilter.chapter}
                        onChange={e => setQFilter(f => ({ ...f, chapter: e.target.value }))}
                        style={smallSelect}
                    >
                    ...
                    <select
                        value={qFilter.approved}
                        onChange={e => setQFilter(f => ({ ...f, approved: e.target.value }))}
                        style={smallSelect}
                    >
                    ...
                    <button onClick={loadAll} style={filterBtn}>
                        Apply
                    </button>
                </div>
            )}
            ...
            {tests.map(test => (
                <div key={test._id} style={cardRow}>
                    <div style={{
                        ...
                        height: 40,
                        borderRadius: 10,
                        background: test.type === "free" ? "rgba(29,158,117,0.15)" : "rgba(59,130,246,0.15)",
                        display: "flex",
                        alignItems: "center",
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
                        <span style={{
                            ...
                            padding: "3px 9px",
                            borderRadius: 20,
                            background: test.isPublished ? "rgba(29,158,117,0.2)" : "rgba(74,85,104,0.3)",
                            color: test.isPublished ? "#1D9E75" : "#718096",
                        }}>
                            {test.isPublished ? "Published" : "Draft"}
                        </span>

                        <button onClick={() => togglePublish(test._id)} style={miniBtn}>
                            {test.isPublished ? "Unpublish" : "Publish"}
                        </button>

                        <button onClick={() => editTest(test)} style={miniBtn}>
                            Edit
                        </button>

                        <button onClick={() => deleteTest(test._id)} style={dangerMiniBtn}>
                            Delete
                        </button>
                    </div>
                </div>
                ...
                <div key={q._id} style={{ background: "#161b27", border: "0.5px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "14px 18px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8 }}>
                                {(q.options || []).map((opt, idx) => (
                                    <div
                                        ...
                                        style={{
                                            fontSize: 11,
                                            color: q.correctIndex === idx ? "#1D9E75" : "#9ca3af",
                                            background: q.correctIndex === idx ? "rgba(29,158,117,0.12)" : "rgba(255,255,255,0.03)",
                                            padding: "5px 8px",
                                            borderRadius: 7,
                                        }}
                                    >
                            ...
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                                ...
                                <span style={{
                                    ...
                                    padding: "2px 8px",
                                    borderRadius: 20,
                                    background: q.difficulty === "hard" ? "rgba(239,68,68,0.15)" : q.difficulty === "easy" ? "rgba(29,158,117,0.15)" : "rgba(234,179,8,0.15)",
                                    color: q.difficulty === "hard" ? "#ef4444" : q.difficulty === "easy" ? "#1D9E75" : "#ca8a04",
                                }}>
                                    {q.difficulty}
                                </span>
                                ...
                                <span style={{
                                    ...
                                    padding: "2px 8px",
                                    borderRadius: 20,
                                    background: q.isApproved ? "rgba(29,158,117,0.15)" : "rgba(234,179,8,0.15)",
                                    color: q.isApproved ? "#1D9E75" : "#ca8a04",
                                }}>
                        ...
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                            ...
                            <button onClick={() => editQ(q)} style={miniBtn}>
                                Edit
                            </button>

                            <button onClick={() => deleteQ(q._id)} style={dangerMiniBtn}>
                                Del
                            </button>
                        </div>
                    </div>
...
function SubjectFormModal({ authAxios, onClose, onSaved }) {
    ...
    const handleSave = async () => {
        ...
        const chapters = chaptersText
            ...
            .map(line => {
                ...
                return {
                    term: chapterPart ? termPart : "",
                    name: chapterPart || termPart,
                };
            });
    ...
    return (
        <ModalShell title="Add Subject" onClose={onClose} maxWidth={620}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <Field label="Subject Name">
                    <input
                        ...
                        style={inp}
                        placeholder="Example: Rog Nidana"
                    />
                </Field>
...
function BulkUploadModal({ authAxios, onClose, onSaved }) {
            ...
            try {
                ...
                if (list.length > 0) {
                    setMeta({
                        subject: list[0].name,
                        term: list[0].terms?.[0] || "",
                        chapter: list[0].chapters?.[0]?.name || "",
                    });
                }
            } catch (e) {
    ...
    const availableChapters = selectedSubject?.chapters?.filter(c => {
        if (!meta.term) return true;
        return c.term === meta.term;
    }) || [];
    ...
    const previewUpload = async () => {
        ...
        try {
            ...
            fd.append("passingScore", form.passingScore);
            fd.append("shuffleQuestions", form.shuffleQuestions);

            // 🔥 IMPORTANT: send preview instead of file
            fd.append("questions", JSON.stringify(previewQuestions));

            const res = await authAxios.post("/tests/questions/preview-upload", fd, {
    ...
    const handleSubjectChange = (subjectName) => {
        ...
        const firstChapter =
            subject?.chapters?.find(c => !firstTerm || c.term === firstTerm)?.name ||
    ...
    const handleTermChange = (term) => {
        const firstChapter =
            selectedSubject?.chapters?.find(c => c.term === term)?.name ||
            selectedSubject?.chapters?.[0]?.name ||
            "";
    ...
    return (
        <ModalShell title="Bulk Upload Questions" onClose={onClose} maxWidth={950}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 14 }}>
                ...
                <div>
                    ...
                    <select
                        value={meta.chapter}
                        onChange={e => {
                            setMeta(m => ({ ...m, chapter: e.target.value }));
                            setQuestions([]);
                        }}
                        style={inp}
                    >
                        ...
                        {availableChapters.map((ch, i) => (
                            <option key={`${ch.name}-${i}`} value={ch.name}>
                                {ch.name}
                            </option>
                        ))}
                    </select>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
                ...
                <button onClick={previewUpload} disabled={loading} style={primaryBtn}>
            ...
            {subjects.length === 0 && (
                <p style={{ color: "#ef4444", fontSize: 13, marginBottom: 12 }}>
                    No subjects found. Add subjects first from backend/admin subject setup.
                </p>
            )}
            ...
            {questions.length > 0 && (
                <>
                    <p style={{ color: "#9ca3af", fontSize: 13, marginBottom: 12 }}>
                        ✅ {questions.length} questions detected. Review/edit before saving.
                    </p>
                        ...
                        {questions.map((q, qi) => (
                            ...
                            >
                                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                                    ...
                                    <button onClick={() => removeQuestion(qi)} style={dangerMiniBtn}>
                                        Remove
                                    </button>
                                </div>
                                ...
                                {q.options.map((opt, oi) => (
                                    <div key={oi} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                                        ...
                                        <span style={{ color: "#9ca3af", width: 18 }}>
                                            {["A", "B", "C", "D"][oi]}
                                        </span>
        ...
        const loadSubjects = async () => {
            try {
                ...
                if (!editing && list.length > 0) {
                    ...
                    const firstChapter =
                        firstSubject.chapters?.find(c => !firstTerm || c.term === firstTerm)?.name ||
                        "";
    ...
    const handleSave = async () => {
        ...
        try {
            if (questionSource === "upload" && !editing) {
                ...
                if (editing) {
                    await authAxios.put(`/tests/${editing._id}`, payload);
                } else {
                    await authAxios.post("/tests", payload);
                }
            }
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <Field label="Term">
                        <select
                            ...
                            onChange={e => {
                                const firstChapter =
                                    selectedSubject?.chapters?.find(c => c.term === e.target.value)?.name ||
                                    "";
                    ...
                    <div style={{ background: "#0f1117", padding: 14, borderRadius: 10, border: "0.5px solid rgba(255,255,255,0.08)" }}>
                        ...
                        <div style={{ display: "flex", gap: 14, marginBottom: 12 }}>
                            <label style={{ color: "#9ca3af", fontSize: 13, cursor: "pointer" }}>
                                <input
                                    type="radio"
                                    checked={questionSource === "bank"}
                                    onChange={() => setQuestionSource("bank")}
                                    style={{ marginRight: 6 }}
                                />
                                Use Question Bank
                            </label>
                            ...
                            <label style={{ color: "#9ca3af", fontSize: 13, cursor: "pointer" }}>
                                <input
                                    type="radio"
                                    checked={questionSource === "upload"}
                                    onChange={() => setQuestionSource("upload")}
                                    style={{ marginRight: 6 }}
                                />
                                Upload DOCX Now
                            </label>
                        ...
                        {questionSource === "upload" && (
                            <div>
                                ...
                                {previewQuestions.length > 0 && (
                                    <p style={{ color: "#1D9E75", fontSize: 12, marginTop: 10 }}>
                                        ✅ {previewQuestions.length} questions detected. Review below before creating test.
                                    </p>
                                )}
                            </div>
                        )}
                                ...
                                {previewQuestions.map((q, qi) => (
                                    <div key={qi} style={{
                                        ...
                                        <textarea
                                            value={q.text}
                                            onChange={e => {
                                                const copy = [...previewQuestions];
                                                copy[qi].text = e.target.value;
                                                setPreviewQuestions(copy);
                                            }}
                                            rows={2}
...
function QuestionFormModal({ authAxios, editing, onClose, onSaved }) {
    const [form, setForm] = useState({
        ...
        subject: editing?.subject || "Swasthavritta evam Yoga",
        term: editing?.term || "Term 1",
    ...
    const handleSave = async (addNext = false) => {
        ...
        try {
            ...
            if (editing) {
                await authAxios.put(`/tests/questions/${editing._id}`, payload);
            } else {
                await authAxios.post("/tests/questions", payload);
            }
    ...
    return (
        <ModalShell title={editing ? "Edit Question" : "Add Question"} onClose={onClose} maxWidth={620}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div>
                    ...
                    {form.options.map((opt, i) => (
                        <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                            ...
                            {form.correctIndex === i && (
                                <span style={{ fontSize: 11, color: "#1D9E75", fontWeight: 700 }}>✓ Correct</span>
                            )}
                        </div>
                    ))}
                </div>
                ...
                <Field label="Tags Optional">
                    <input
                        ...
                        style={inp}
                        placeholder="vata, herbs, classic…"
                    />
                </Field>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
                ...
                {!editing && (
                    <button onClick={() => handleSave(true)} disabled={saving} style={{ ...secondaryBtn, flex: 1 }}>
                        Save & Add Next
                    </button>
                )}

                <button onClick={() => handleSave(false)} disabled={saving} style={{ ...primaryBtn, flex: 1.5 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
            <div style={{ background: "#161b27", borderRadius: 16, padding: 28, width: "100%", maxWidth, border: "0.5px solid rgba(255,255,255,0.1)", maxHeight: "90vh", overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 20 }}>
                    ...
                    <button onClick={onClose} style={{ background: "transparent", border: "none", color: "#9ca3af", fontSize: 20, cursor: "pointer" }}>
                        ×
                    </button>
                </div>
